...
"""
    ...
    def _perform_analysis(self, df: pd.DataFrame, task: str) -> str:
        ...
        if 'roas' in task.lower() or 'drop' in task.lower() or 'change' in task.lower():
            ...
            prev_7_days = df[(df['date'] <= max_date - timedelta(days=7)) & 
                           (df['date'] > max_date - timedelta(days=14))]
            
            if len(last_7_days) > 0 and len(prev_7_days) > 0:
                recent_roas = (last_7_days['revenue'].sum() / 
                             last_7_days['spend'].sum() if last_7_days['spend'].sum() > 0 else 0)
                prev_roas = (prev_7_days['revenue'].sum() / 
                           prev_7_days['spend'].sum() if prev_7_days['spend'].sum() > 0 else 0)
                
                roas_change = ((recent_roas - prev_roas) / prev_roas * 100 
                             if prev_roas > 0 else 0)
                
                analysis.append(f"ROAS Trend:\n- Last 7 days: {recent_roas:.2f}\n- Previous 7 days: {prev_roas:.2f}\n- Change: {roas_change:+.1f}%")
        
        # CTR analysis
        if 'ctr' in task.lower() or 'click' in task.lower():
            low_ctr_threshold = self.config['thresholds']['low_ctr_threshold']
            low_ctr_campaigns = df[df['ctr'] < low_ctr_threshold].groupby('campaign_name').agg({
                'ctr': 'mean',
                'spend': 'sum',
                'impressions': 'sum'
            }).sort_values('spend', ascending=False).head(5)
    ...
    def get_low_ctr_campaigns(self) -> pd.DataFrame:
        ...
        low_ctr = df[df['ctr'] < threshold].groupby(['campaign_name', 'creative_message']).agg({
            'ctr': 'mean',
            'spend': 'sum',
            'roas': 'mean'
    ...
    def get_top_performers(self) -> pd.DataFrame:
        ...
        top_perf = df.groupby(['creative_type', 'creative_message']).agg({
            'ctr': 'mean',
            'roas': 'mean',
            'spend': 'sum'
        }).reset_index()
